...
`ifdef MAC_BASIC_FLOW_TEST
    ...
    static logic signed [39:0] expected_acc = 0;
        ...
        $display("[TB] Ejecutando ráfaga final para 100%% de cobertura cruzada");
         ...
         // --- Cruces con CERO ---
        if_i.compute(16'd0,16'd100);// Bin: (zero, pos)
        if_i.compute(16'd0,-16'd100);// Bin: (zero, neg)
        ...
        if_i.compute(16'sh7FFF,16'd50);// Bin: (max_pos, pos)
        if_i.compute(16'sh7FFF,-16'd50);// Bin: (max_pos, neg)
        if_i.compute(16'sh8000,16'd50);// Bin: (max_neg, pos)
        if_i.compute(16'sh8000,-16'd50);// Bin: (max_neg, neg)

        $display("\nTEST CASE 1 PASSED (Total: 104 pruebas)");
	
	// ==========================================================
        // TEST CASE 6: RESET ON-FLY (Ahora va en fila, sin interrumpir)
        // ==========================================================
        $display("\n[TB] =========================================");
